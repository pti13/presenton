...
class TestImageGenerationService:
            ...
            with patch('services.image_generation_service.is_pixels_selected', return_value=False):
                with patch('services.image_generation_service.is_gemini_flash_selected', return_value=False):
                    with patch('services.image_generation_service.is_dalle3_selected', return_value=False):
                        with patch('services.image_generation_service.is_stable_diffusion_selected', return_value=False):
    ...
    def test_get_image_gen_func_stable_diffusion_selected(self, mock_images_directory):
        """
        Test function selection when Stable Diffusion is selected
        - Checks if the correct function is selected based on environment variable
        - Ensures that the function is set to generate_image_stable_diffusion when Stable Diffusion is selected
        """
        with patch('services.image_generation_service.is_pixabay_selected', return_value=False):
            with patch('services.image_generation_service.is_pixels_selected', return_value=False):
                with patch('services.image_generation_service.is_gemini_flash_selected', return_value=False):
                    with patch('services.image_generation_service.is_dalle3_selected', return_value=False):
                        with patch('services.image_generation_service.is_stable_diffusion_selected', return_value=True):
                            with patch.dict(os.environ, {"IMAGE_PROVIDER": "stable_diffusion"}):
                                service = ImageGenerationService(mock_images_directory)
                                assert service.image_gen_func == service.generate_image_stable_diffusion
    ...
    def test_generate_image_with_stable_diffusion_success(self, mock_images_directory, sample_image_prompt):
        """
        Test successful image generation with Stable Diffusion provider
        - Mocks the Stable Diffusion API to return a valid image
        - Ensures that the image generation function returns the expected ImageAsset
        - Checks if the image generation function is called with the correct prompt
        """
        async def run_test():
            with patch.dict(os.environ, {"IMAGE_PROVIDER": "stable_diffusion", "STABLE_DIFFUSION_URL": "http://localhost:7860"}):
                with patch('services.image_generation_service.is_pixels_selected', return_value=False):
                    with patch('services.image_generation_service.is_pixabay_selected', return_value=False):
                        with patch('services.image_generation_service.is_gemini_flash_selected', return_value=False):
                            with patch('services.image_generation_service.is_dalle3_selected', return_value=False):
                                with patch('services.image_generation_service.is_stable_diffusion_selected', return_value=True):
                                    ...
                                    # Create a real test file
                                    test_image_path = f"{mock_images_directory}/test_stable_diffusion_image.png"
                                    with open(test_image_path, 'w') as f:
                                        f.write("fake image content")
                                    
                                    # Mock generate_image_stable_diffusion to return the test file path
                                    async def mock_stable_diffusion_generate(prompt, output_dir):
                                        return test_image_path
                                    
                                    service.generate_image_stable_diffusion = mock_stable_diffusion_generate
